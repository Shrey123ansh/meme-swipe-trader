...
import { TrendingUp, BarChart3, Wallet, Copy, Users, MessageSquare, Home, Plus, User, Shield } from 'lucide-react';
...
export default function Layout({ children }: LayoutProps) {
  ...
  const connectWallet = async () => {
    if (typeof window.ethereum !== 'undefined') {
      ...
      // Enhanced wallet installation guidance
      const installWallet = confirm(
        'MetaMask not detected. Would you like to install it?\n\n' +
        'Click OK to visit the MetaMask download page.'
      );
      if (installWallet) {
      ...
      <div className="narrow-navbar mx-auto border-b border-border">
        <div className="px-6 h-14 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              {/* Wallet Status with Security Indicators */}
              {isConnected ? (
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link 
                    ...
                    to={item.path} 
                    className="tab-item"
                    data-tour={item.path === '/' ? 'home-tab' : undefined}
                  >
                    <motion.div
                    ...
                    >
                      ...
                      {isActive && (
                        <motion.div
                          className="absolute -top-1 w-1 h-1 bg-primary-foreground rounded-full"
                          initial={{ scale: 0 }}
                          animate={{ scale: 1 }}
